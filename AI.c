...
void menu(){
    ...
    if(choix == 1) jeu_humains();
        ...
        printf("Voulez-vous jouer contre une ia simple (1), ou contre une ia avancée (2) ?\n>");
        scanf("%d", &choix);
    } while(choix < 1 || choix > 2);
    ...
    if(choix == 1) jeu_ia_simple(choix_tour());
        ...
        do{
            printf("À quelle niveau de difficulté voulez-vous jouer ?\n>");
            scanf("%d", &choix);
        } while(choix < 1 || choix > 10);

        jeu_ia_minmax(choix, choix_tour());
    }
}
...
void jeu_ia_minmax(int depth, int ia){
    ...
    do{
        printf("\n");
        if(tour+1 == ia) tour_ia_minmax(grille, ia, depth); //l'IA joue seulement quand c'est son tour (ça marche, fais une table de vérité)
        else tour_humains(grille, (ia%2)+1); //si c'est au tour du joueur, on lui fait jouer avec le char INVERSE de celui de l'IA
        print_grille(grille);
        tour = !tour;
        printf("\n");
...
void tour_ia_minmax(char** grille, int ia, int depth){
    int scores[7] = {0};
    for(int i = 0; i < 7; i++){
        scores[i] = minmax(grille, ia, depth, i);
    }

    if(check_equal(scores)) tour_ia_simple(grille, ia);
    else {
        int max = find_max(scores);
        place_jeton(grille, ia, max);
        printf("L'ordinateur a joué à la colonne n°%d\n", max);
    }
}
